...
wiki50_vocab_addr = os.path.join(base_addr, 'files/wiki50_vocab.dat')
UPC_VOCAB_ADDR = os.path.join(base_addr, 'files/word_pos_UPC.txt')
POSTFIX_MAPPER_ADDR = os.path.join(base_addr, 'files/tokenizer/postfix_mapper.csv')
ISOLATED_WORDS_ADDR = os.path.join(base_addr, 'files/isolate_words')
POSTFIX_FREQ_ADDR = os.path.join(base_addr, 'files/tokenizer/informal_postfixs_freq')
POSTFIXS_ADDR = os.path.join(base_addr, 'files/tokenizer/informal_postfixs.txt')
VERBS_CSV_ADDR = os.path.join(base_addr, 'files/verbs.csv')
INFORMAL_IN_FORMAL_WORDS_ADDR = os.path.join(base_addr, 'files/informal_words_in_formal_corpus.txt')
ENDS_WITH_TANVIN_WORDS_ADDR = os.path.join(base_addr, 'files/spell/end_with_tanvin.txt')
SPELL_CHECKING_MAPPER_ADDR = os.path.join(base_addr, 'files/spell/spell_checker_mapper.json')
IRREGULAR_VERBS_MAPPRER_ADDR = os.path.join(base_addr, 'files/irregular_verb_mapper.csv')
...
def translate():
    global model
    # txts = [ 'خوندش کتابو', 'خوشبخته واقعا اون', 'همینه که میگم باید بیارینشون اینجا']
    data = flask.request.json
    txts = data['texts']
    outputs = {}
    log_f_addr = os.path.join(base_addr, 'files/logs.log')
    logs_f = open(log_f_addr, 'a+')
    for txt_index, origin_txt in enumerate(txts):
        try:
            ...
            outputs[txt_index] = 'ERROR ' + str(e)
        log = f'input:\t{origin_txt}\noutput:\t{outputs[txt_index]}\n\n'
        logs_f.write(log)
    return outputs

@app.route('/', methods=['GET', 'POST'])
def home():
    return 'welcome informal2formal service'

global model
model = Informal2Formal()
if __name__ == '__main__':
    app.run(port='6231')
    # translate(model, open('test.txt').read().splitlines())
#
