...
class InformalTokenizer:
    def __init__(self, vocab, informal_postfixs_addr):
        self.vocab = vocab
        self.pres = InformalTokenizer.get_prefixs()
        self.posts = InformalTokenizer.get_postfixs(informal_postfixs_addr)
    ...
    def get_postfixs(informal_postfix_addr):
        ...
        # p2 = ['ها', 'ا','']
        # p3 = ['ی','']
        # p4 = ['م', 'ت', 'ش', 'مان', 'مون', 'تان', 'تون', 'شان', 'شون','']
        # p5 = ['و','ه', '']
        # postfixs_cobinations = itertools.product(p1, p2, p3, p4, p5)
        # posts = [''.join(pc) for pc in postfixs_cobinations]
        # posts.remove('و')
        # posts.remove('مو')
        # posts.append('ای')
        # return posts
    ...
    def get_candidates(self, tokens, index=0, current_seq = ' '):
        prefixs = ['بی', 'نا']
        postfixs = ['م', 'ت', 'ام', 'ش', 'اش', 'مان', 'مون', 'تان', 'تون', 'شان', 'شون', 'ه', 'ی', 'ای', 'ات', 'ها']
        'baraye spell checker valid bashan '

        'کتاب ها م را می برند '
        'کتابا شونو امروز بردن'
        'امروز به یک شو معرکه دعوت شدم'
        'امروز کلاس شو واقعا دوست داشتم '
        ''
        if index == len(tokens):
            return current_seq
        word = tokens[index]
