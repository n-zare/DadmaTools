...
from symspellpy.symspellpy.editdistance import DistanceAlgorithm, EditDistance
import symspellpy.symspellpy.helpers as helpers
...
class SymSpell(object):
    ...
    def lookup(self, phrase, verbosity, max_edit_distance=None,
               include_unknown=False, is_valid=None,
               transfer_casing=False, original_words=None):
        ...
        if is_valid(phrase) and phrase in original_words:
            suggestion_count = 1
            suggestions.append(SuggestItem(phrase, 0, suggestion_count))
            # early exit - return exact match, unless caller wants all
            # matches
        ...
        for j in range(len(phrase)):
            ...
            for i in range(1, imax + 1):
                ...
                results = self.lookup(part, Verbosity.TOP, max_edit_distance,
                                      is_valid=is_valid, original_words=original_words)
                if results:
